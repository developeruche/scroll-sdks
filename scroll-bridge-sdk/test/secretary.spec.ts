...
describe('Secretory Module', async () => {
    describe('getMessageStatus', async () => {
        it('should test getting message status on TESTNET (L1) -> CONFIRMED', async () => {
            let currentMessageStatus = await getMessageStatus(CHAIN.L1, TEST_L1_MESSAGE_HASH_VALID_TESTNET, providerL1, providerL2, true);
            expect(currentMessageStatus).to.be.equal(MESSAGE_STATUS.CONFIRMED);
        })

        it('should test getting message status on TESTNET (L1) -> CONFIRMED', async () => {
            let currentMessageStatus = await getMessageStatus(CHAIN.L1, TEST_L1_MESSAGE_HASH_INVALID_TESTNET, providerL1, providerL2, true);
        ...
        it('should test getting message status on TESTNET (L2) -> CONFIRMED', async () => {
            let currentMessageStatus = await getMessageStatus(CHAIN.L2, TEST_L2_MESSAGE_HASH_VALID_TESTNET, providerL1, providerL2, true);
            expect(currentMessageStatus).to.be.equal(MESSAGE_STATUS.CONFIRMED);
        })

        it('should test getting message status on TESTNET (L2) -> PENDING', async () => {
            let currentMessageStatus = await getMessageStatus(CHAIN.L2, TEST_L2_MESSAGE_HASH_INVALID_TESTNET, providerL1, providerL2, true);
            expect(currentMessageStatus).to.be.equal(MESSAGE_STATUS.PENDING);
        })
    })
